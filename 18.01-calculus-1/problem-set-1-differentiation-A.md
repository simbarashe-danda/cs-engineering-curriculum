...
**Problem 1A-1 b)** By completing the square, use translation and change of scale to sketch
...
$$y = 3\left(x^2 + 2x + \tfrac{2}{3}\right).$$
...
$$y = 3\left((x+1)^2 - \tfrac{1}{3}\right) = 3(x+1)^2 - 1.$$
...
**Problem 1A-2 b)** Sketch, using translation and change of scale,
...
**Problem 1A-3 a)** Identify whether the function is even, odd, or neither:
...
**Problem 1A-3 e)** Identify whether the function is even, odd, or neither:
...
**Problem 1A-6 b)** Express $\sin x - \cos x$ in the form $A\sin(x+c)$.
...
**Problem 1A-7 b)** Find the period, amplitude, and phase angle, and use these to sketch

$$y = -4\cos\left(x+\frac{\pi}{2}\right).$$
...
![Graph of the translated function](./images/trig.png)

---

## 1B-1 Velocity and Rates of Change

**Problem 1B-1 a) & b)** A test tube falls from a tower 400 feet high, dropping $16t^2$ feet in $t$ seconds.
...
**Problem 1C-1 a)** Use the difference‑quotient definition to calculate the rate of change of the area of a disk with respect to its radius.
...
A'(r) &= \lim_{\Delta r\to 0} \frac{\pi(r+\Delta r)^2 - \pi r^2}{\Delta r} \\
      ...
      &= \lim_{\Delta r\to 0} \frac{\Delta r(2\pi r + \pi\Delta r)}{\Delta r}
         \qquad\text{(cancel }\pi r^2\text{ and factor }\Delta r\text{)} \\
      &= \lim_{\Delta r\to 0} (2\pi r + \pi\Delta r) \\
      &= 2\pi r \qquad (\text{as }\Delta r\to0,\ \pi\Delta r\to0).
\end{aligned}
$$
...
**Problem 1C-2** Let $f(x) = (x-a)\,g(x)$. Using the definition of the derivative, show that $f'(a) = g(a)$, assuming $g$ is continuous.
...
f'(a) &= \lim_{\Delta x\to 0} \frac{f(a+\Delta x) - f(a)}{\Delta x} \\
      &= \lim_{\Delta x\to 0} \frac{\Delta x \cdot g(a+\Delta x) - 0}{\Delta x}
         \qquad\bigl(f(a)=0,\; f(a+\Delta x)=\Delta x\,g(a+\Delta x)\bigr) \\
      &= \lim_{\Delta x\to 0} g(a+\Delta x) \\
      &= g(a) \qquad (\text{since }g\text{ is continuous}).
\end{aligned}
$$
...
**Problem 1C-3 b)** Calculate the derivative of $f(x) = 2x^2 + 5x + 4$ directly from the definition.
...
Inserting the function and expanding $(x+\Delta x)^2 = x^2 + 2x\Delta x + (\Delta x)^2$:

$$
\begin{aligned}
f'(x) &= \lim_{\Delta x\to 0} \frac{f(x+\Delta x)-f(x)}{\Delta x} \\
      &= \lim_{\Delta x\to 0} \frac{\left[2(x^2+2x\Delta x+(\Delta x)^2) + 5x+5\Delta x + 4\right] - \left[2x^2+5x+4\right]}{\Delta x} \\
      &= \lim_{\Delta x\to 0} \frac{4x\Delta x + 2(\Delta x)^2 + 5\Delta x}{\Delta x} \\
      &= \lim_{\Delta x\to 0} \frac{\Delta x(4x + 2\Delta x + 5)}{\Delta x} \\
      &= \lim_{\Delta x\to 0} (4x + 2\Delta x + 5) \\
      &= 4x + 5.
\end{aligned}
$$
...
**Problem 1C-3 a)** Calculate the derivative of $f(x) = \dfrac{1}{2x+1}$ directly from the definition.
...
f'(x) &= \lim_{\Delta x\to 0} \frac{ \frac{1}{2(x+\Delta x)+1} - \frac{1}{2x+1} }{\Delta x} \\
      &= \lim_{\Delta x\to 0} \frac{ \frac{-2\Delta x}{(2x+2\Delta x+1)(2x+1)} }{\Delta x}
         \qquad\left(\text{combining fractions}\right) \\
...
$$y - \frac13 = -\frac{2}{9}(x-1) \;\Longrightarrow\; y = -\frac{2}{9}x + \frac{2}{9} + \frac{3}{9} = \frac{-2x+5}{9}.$$

---

## 1C-5 Tangent Lines Through the Origin

**Problem 1C-5** Find all tangent lines through the origin to the graph of $y = 1 + (x-1)^2$.
...
**Problem 1D-1 a), d), f), g)** Calculate the following limits if they exist.
...
**(d)** As $x\to 2^+$, the denominator $2-x$ approaches $0$ from the negative side (tiny negative), while the numerator approaches $16$. Hence the fraction tends to $-\infty$:

$$\lim_{x\to 2^+}\frac{4x^2}{2-x} = -\infty.$$

**(f)** Dividing numerator and denominator by $x$:

$$\frac{4x^2}{x-2} = \frac{4x}{1 - 2/x}.$$

As $x\to\infty$, $4x\to\infty$ and $1-2/x\to 1$; therefore the limit is $+\infty$.

**(g)** Combining into a single fraction:
...
**Problem 1D-3 a) & c)** Identify and classify the points of discontinuity.
...
**Problem 1D-8 a)** Find values of $a$ and $b$ such that the function is continuous but not differentiable:
...
**Problem 1E-4 b)** Find all $a, b$ for which the function is differentiable:
...
$$
\begin{aligned}
\lim_{x\to1^-} f(x) &= a(1)^2 + b(1) + 4 = a+b+4, \\
\lim_{x\to1^+} f(x) &= 5+3+7+8+4 = 27.
\end{aligned}
$$
...
$$
a+b+4 = 27 \quad\Longrightarrow\quad a+b = 23. \qquad (1)
$$
...
Thus the function is differentiable only when
...
**Problem 1E-1 c)** Find the derivative: $\displaystyle \frac{d}{dx}\left(\frac{x}{2} + \pi^3\right)$.
...
**Problem 1E-2 b)** Find an antiderivative of $f'(x) = x^6 + 5x^5 + 4x^3$.
...
**Problem 1E-3)** Find the points $(x,y)$ on the graph of $y = x^3 + x^2 - x + 2$ where the tangent line is horizontal.
...
**Problem 1E-5 a)** Find the derivative of $\displaystyle \frac{x}{1+x}$.
...
**Problem 1E-5 c)** Find the derivative of $\displaystyle \frac{x+2}{x^2-1}$.

**Solution**

Quotient rule:

$$\frac{(x^2-1)(1) - (x+2)(2x)}{(x^2-1)^2} = \frac{x^2 - 1 - 2x^2 - 4x}{(x^2-1)^2} = \frac{-x^2 - 4x - 1}{(x^2-1)^2}.$$

---

## 1J-1 & 1J-2 Trigonometric Derivatives and Limits

**Problem 1J-1 e)** Calculate the derivative: $\displaystyle \frac{d}{dx}\left(\frac{\sin x}{x}\right)$.
...
**Problem 1J-2** Evaluate the limit by relating it to a derivative value:
...
**Problem 1F-1 a) & b)** Find the derivatives.
...
**Problem 1F-2** Find the derivative of $x^{10}(x^2+1)^{10}$.
...
**Problem 1F-7 b) & c)** (b) For $m = \dfrac{m_0}{\sqrt{1 - v^2/c^2}}$, find $dm/dv$.
...
$$\frac{dm}{dv} = m_0\left(-\frac12\right)(1 - v^2/c^2)^{-3/2}\left(-\frac{2v}{c^2}\right) = \frac{m_0 v}{c^2}\,(1 - v^2/c^2)^{-3/2} = \frac{v m_0}{c^2(1 - v^2/c^2)^{3/2}}.$$

**(c)** Write $F = mg(1+r^2)^{-3/2}$.

Chain rule:

$$\frac{dF}{dr} = mg\left(-\frac32\right)(1+r^2)^{-5/2}\cdot 2r = -3mgr\,(1+r^2)^{-5/2} = \frac{-3mgr}{(1+r^2)^{5/2}}.$$

---

## 1G-1 Higher Derivatives

**Problem 1G-1 c)** Calculate $y''$ for $y = \dfrac{x}{x+5}$.
...
**Problem 1G-5 a)** Let $y = u(x)v(x)$. Find $y'$, $y''$, and $y'''$.
...
**Problem 1F-6** Show that the derivative of an even function is odd, and the derivative of an odd function is even.
