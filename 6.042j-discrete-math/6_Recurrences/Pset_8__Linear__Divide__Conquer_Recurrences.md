...
Then $T(x) = \Theta\left(x^p \left(1 + \int_{1}^{x} \frac{g(u)}{u^{p+1}} du\right)\right)$, where $p$ satisfies the equation $\sum_{i=1}^{k} a_i b_i^p = 1$.
...
Dropping linear growth terms and constants:
$T(x) = \Theta(x^3)$.
Thus, $T(n) = \Theta(n^3)$.
...
Using the theorem from lecture: If $g(x) = \Theta(x^t)$ for $t > 0$ and $\sum_{i=1}^{k} a_i b_i^t < 1$, then $T(x) = \Theta(g(x))$.
...
$h_1(x) = \lfloor \frac{n}{4} \rfloor - \frac{n}{4} + 1 \le 1$. Thus $h_1(x) = O(x / \log^2 x)$.
...
Notice that $\lfloor x/9 \rfloor - x/9 \le 1 \ \forall x \ge 0$. Let $S(x) = \lfloor x/9 \rfloor - \frac{x}{9}$.
$$\lim_{x \to \infty} \frac{\log^2 x \cdot S(x)}{x} + \frac{\log^2 x \cdot x^{1/9}}{x}$$
For both fractions, $x$ grows faster. Hence the limit is $>0$ (approaches 0). Thus $h_1(x) = O(x / \log^2 x)$.
...
Then by $P(n/2)$, $T(n) \le 4c(n/2) + n$.
Then $T(n) \le (2c + 1)n$.

We need to find a contradiction that $T(n) \le cn$. Since by our assumption $c > 0$, then $T(n) \le (2c + 1)n$ is a true statement. We can't establish a contradiction from it.
...
**Proof:**
Notice that $a_1 = a$, $b_1 = \frac{1}{b}$, $g(n) = n$ for $T(\lfloor n/b \rfloor) + n$.
...
$A_0 = 2$
$A_{n+1} = A_n/2 + 1/A_n$ (for $n \ge 1$)
Prove that $A_n \le \sqrt{2} + 1/2^n$ for all $n \ge 0$.
...
$$A_{n+1} \le \frac{\sqrt{2} + \frac{1}{2^n}}{2} + \frac{1}{\sqrt{2}} \quad (\text{Since } A_n \ge \sqrt{2} \implies \frac{1}{A_n} \le \frac{1}{\sqrt{2}})$$
...
### Part (a)
**Question:**
$x_n = 4x_{n-1} - x_{n-2} - 6x_{n-3} \quad (x_0 = 3, x_1 = 4, x_2 = 14)$

**Solution:**
Try $x_n = \alpha^n$ for a constant $\alpha$.
Then $\alpha^n - 4\alpha^{n-1} + \alpha^{n-2} + 6\alpha^{n-3} = 0$.
Dividing by $\alpha^{n-3}$: $\alpha^3 - 4\alpha^2 + \alpha + 6 = 0$.
...
Therefore, $(\alpha - 2)(\alpha^2 - 2\alpha - 3) = 0$.
Factorizing $\alpha^2 - 2\alpha - 3 = (\alpha - 3)(\alpha + 1)$.
The roots are $\alpha_1 = 2$, $\alpha_2 = 3$, $\alpha_3 = -1$.

We know that if $f(n) = \alpha^n$ is a solution, then $x_n = C_1(2)^n + C_2(3)^n + C_3(-1)^n$ is also a solution.

Using boundary conditions:
$x_0 = 3 \implies C_1(2)^0 + C_2(3)^0 + C_3(-1)^0 \implies C_1 + C_2 + C_3 = 3$ (Eq 1)
$x_1 = 4 \implies 2C_1 + 3C_2 - C_3 = 4$ (Eq 2)
$x_2 = 14 \implies 4C_1 + 9C_2 + C_3 = 14$ (Eq 3)

Solving the system:
Adding Eq 1 and Eq 2: 
$(C_1 + C_2 + C_3) + (2C_1 + 3C_2 - C_3) = 3 + 4 \implies 3C_1 + 4C_2 = 7$ (Eq A)

Adding Eq 2 and Eq 3: 
$(2C_1 + 3C_2 - C_3) + (4C_1 + 9C_2 + C_3) = 4 + 14 \implies 6C_1 + 12C_2 = 18 \implies C_1 + 2C_2 = 3$ (Eq B)

Solving for $C_1$ and $C_2$:
Multiplying Eq B by 2 gives $2C_1 + 4C_2 = 6$.
Subtracting this result from Eq A: $(3C_1 + 4C_2) - (2C_1 + 4C_2) = 7 - 6 \implies C_1 = 1$.
Substitute $C_1$ into Eq B: $1 + 2C_2 = 3 \implies 2C_2 = 2 \implies C_2 = 1$.
Solve for $C_3$: Substitute $C_1=1$ and $C_2=1$ into Eq 1: $1 + 1 + C_3 = 3 \implies C_3 = 1$.

Thus, $x_n = 2^n + 3^n + (-1)^n$.

### Part (b)
**Question:**
$x_n = -x_{n-1} + 2x_{n-2} + n \quad (x_0 = 5, x_1 = -4/9)$

**Solution:**
Finding homogenous solutions:
Solving $x_n + x_{n-1} - 2x_{n-2} = 0$. Try $x_n = \alpha^n$.
Then $\alpha^n + \alpha^{n-1} - 2\alpha^{n-2} = 0$.
Then $\alpha^2 + \alpha - 2 = 0$.
The roots are $\alpha_1 = -2$ and $\alpha_2 = 1$.
Then $x_n^{(h)} = C_1(-2)^n + C_2(1)^n$.

Finding particular solutions to $x_n = -x_{n-1} + 2x_{n-2} + n$:
Since $f(n) = n$ is a polynomial of degree 1, the standard base guess is $an+b$. By the multiplicity rule, since 1 is a root of the characteristic equation with multiplicity 1, we must multiply our guess by $n$.
Guess: $x_n^{(p)} = n(an+b) = an^2 + bn$.
...
Thus $x_n^{(p)} = \frac{3n^2 + 7n}{18}$.

Adding particular and homogenous solutions gives the General Solution:
$x_n = C_1(-2)^n + C_2 + \frac{3n^2 + 7n}{18}$

Using boundary conditions:
$x_0 = C_1 + C_2 = 5 \implies C_2 = 5 - C_1$ (Eq 1)
$x_1 = -2C_1 + C_2 + \frac{3+7}{18} = -4/9$
$-2C_1 + C_2 + 10/18 = -8/18 \implies -2C_1 + C_2 = -1$ (Eq 2)

From (1) and (2):
$-2C_1 + (5 - C_1) = -1 \implies -3C_1 = -6 \implies C_1 = 2$.
$C_2 = 5 - 2 = 3$.

Thus, $x_n = 3 + 2(-2)^n + \frac{3n^2 + 7n}{18}$.