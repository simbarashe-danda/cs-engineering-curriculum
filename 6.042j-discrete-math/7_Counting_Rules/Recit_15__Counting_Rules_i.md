...
**Solution:**
Consider selecting a letter for the first slot. Notice that there are 4 choices. For the second slot, there are 3 choices since a letter has already been selected for the first. By the generalized product rule, there are $4 \cdot 3 \cdot 2 \cdot 1 = 4!$ ways to arrange. 

Therefore, there are 24 possible ways to arrange the letters in $POKE$.
...
**Solution:**
There are 4 possible first entries, 3 possible second entries, etc. Therefore, there are $4! = 24$ ways to arrange the letters in $BO_1O_2K$.
...
**Solution:**
Let $A$ be the set of all arrangements of $BO_1O_2K$. Let $B$ be the set of all arrangements of $BOOK$. Let $f$ be the mapping $f: A \rightarrow B$.

Then $f$ is surjective since $\forall L_y \in B, \exists L_x \in A \text{ s.t. } f(L_x) = L_y$. This is a $2\text{-to-}1$ mapping.
...
**Solution:**
By the division rule, if $f: A \rightarrow B$ is $k\text{-to-}1$, then $|A| = k \cdot |B|$. 
...
Hence, there are $\frac{4!}{2}$ ways to arrange $BOOK$.
...
**Solution:**
By the generalized product rule, there are 6 possible first entries, 5 possible second entries... 1 possible last entry. Hence, there are $6!$ ways to arrange $KE_1E_2PE_3R$.
...
Listing them: 
$RE_1PE_2E_3K$, $RE_1PE_3E_2K$, $RE_2PE_1E_3K$, $RE_3PE_2E_1K$, $RE_3PE_1E_2K$, $RE_2PE_3E_1K$.
...
**Solution:**
It follows from 1.7 that this is a $3!\text{-to-}1$ mapping.
...
Therefore, by the division rule, there are $\frac{6!}{3!}$ ways to arrange $KEEPER$.
...
**Solution:**
By the generalized product rule, there are $10!$ ways to arrange $BO_1O_2K_1K_2E_1E_2PE_3R$.
...
**Solution:**
Notice that the mapping from arrangements of $BO_1O_2K_1K_2E_1E_2PE_3R$ to the set of arrangements of $BOOK_1K_2E_1E_2PE_3R$ is a $2\text{-to-}1$ mapping. Since there are $2!$ ways to arrange $O_1$ and $O_2$, then by the division rule, there are $\frac{10!}{2!}$ ways to arrange $BOOK_1K_2E_1E_2PE_3R$.
...
Hence, there are $\frac{10!}{(2!)^2}$ ways to arrange $BOOKKE_1E_2PE_3R$.
...
Hence, there are $\frac{10!}{(2!)^2 \cdot 3!}$ ways to arrange $BOOKKEEPER$.
...
**Solution:**
By generalizing 1.13, there are $\frac{10!}{5! \cdot (2!)^2}$ ways to arrange $VOODOODOLL$.
...
**Solution:**
From $\frac{n!}{k!(n-k)!}$, it follows that there are $\binom{n}{k}$ $n$-bit sequences that contain $k$ zeros and $(n-k)$ ones.
...
**Solution:**
**Generalized Pigeonhole Principle (GPP):** If $|X| > k|Y|$, then for every function $f: X \rightarrow Y$ there exist $k+1$ different elements of $X$ that are mapped to the same element of $Y$.

Let $X$ be the set of 500 people. Let $Y$ be the set of 365 birthdays. It follows that $|X| > 1 \cdot |Y|$. From the theorem, there are $1+1 = 2$ people $\in X$ that share the same birthday.
...
It then follows from the Generalized Pigeonhole Principle that $\exists s_a, s_b \in X$ such that $f(s_a) = f(s_b)$.
...
It follows from GPP that $\exists x_1, x_2 \in X$ such that $f(x_1) = f(x_2) \implies x_1 \pmod{37} = x_2 \pmod{37} \implies x_1 \equiv x_2 \pmod{37}$.
...
Therefore, there are $\binom{16}{12}$ different ways. Alternatively, $\binom{16}{4} = \binom{16}{12}$ different ways.
...
Thus $|Y| = \frac{8!}{2! \cdot 2! \cdot 2!}$ by the division rule.
...
Thus $|Y| = \frac{104!}{2^{52}}$. There are $\frac{104!}{2^{52}}$ ways to arrange a double-sized deck.
...
Thus $25^2(25^2 + 8 \cdot 25 + 8^2 + 2 \cdot 8 \cdot 25) = 614,025$ different 4-phoneme words.
...
Thus using the sum rule, $|A| = \sum_{k=0}^{n/2} |A_k|$ where $A_k$ is the set of all $n$-phoneme words with exactly $k$ consonants.
...
Thus $|A_k| = 8^k \cdot 25^{n-k} \binom{n-k}{k}$.
...
$|A|$ is the set of all $n$-phoneme words in Hawaiian.