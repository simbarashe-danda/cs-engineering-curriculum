...
**Inductive step:** We must show that $P(n)$ implies $P(n+1)$ for all $n \ge 2$. Consider an $n$-vertex graph in which every vertex has positive degree. By the assumption $P(n)$, this graph is connected; that is, there is a path between every pair of vertices. Now we add one more vertex $x$ to obtain an $(n+1)$-vertex graph: obtain a path from $x$ to $z$ by going from $x$ to $y$ and then following the path from $y$ to $z$. This proves $P(n+1)$.
...
**False claim:** For a graph $G = (V,E)$,  
$\forall n \in V, \text{deg}(n) > 0 \implies \text{connected}$

**Counterexample:** Consider a graph $G = (V,E)$  
...
**Case 1:** Original path between $X$ and $Y$ in $T$ did not utilize the edge $e'$.  
Then removing the $e'$ does not make $X$ and $Y$ disconnected from each other.  
**Case 2:** Original path between $X$ and $Y$ did utilize $e'$.  
...
**Lemma 5.** Define $S_m$ to be the set consisting of the first $m$ edges selected by ALG-GROW from a connected graph $G$. Let $P(m)$ be the predicate that if $m \le |V|$ then $S_m \subseteq E$ for some MST $T = (V,E)$ of $G$. Then $\forall m \ . \ P(m)$.  
