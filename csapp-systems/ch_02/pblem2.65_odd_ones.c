...
/* This comes from realization that given bit sequence Y = [b3, b2, b1, b0 ],
b3^b2^b1^b0 will determine parity of the number of 1s in Y.
But doing this linearly is inefficient , O(N) time where N = word size W
...
in other words we appky XOR of half the bits which are right most bits against half which left most.

Final realization was that shifting Y' >> k and appling xor until k = 1 result in least significant bit,
Given Z = Y' ^ (Y >> 1)
z0 represents our desired expression, that is to say
z0 = (b_w-1 ^ b_w-2 ^ ....^ b_1 ^ b_0).

This is O(log_2_N) time where N = word size W
*/
